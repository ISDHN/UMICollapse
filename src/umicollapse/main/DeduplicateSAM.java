...
public class DeduplicateSAM {
    ...
    public void deduplicateAndMerge(File in, File out, Algo algo, Class<? extends Data> dataClass, Merge merge,
            int umiLengthParam, int k, float percentage, boolean parallel, String umiSeparator, boolean paired,
            boolean removeUnpaired, boolean removeChimeric, boolean keepUnmapped, boolean trackClusters) {
        ...
        recordStream.forEach(record -> {
            ...
            // record.getReadNegativeStrandFlag() ? record.getUnclippedEnd() :
        ...
        final Map<Alignment, ClusterTracker> clusterTrackers = /*
                                                                * trackClusters ? new HashMap<Alignment,
                                                                * ClusterTracker>() :
                                                                */ null;

        Stream<Map.Entry<Alignment, Map<BitSet, ReadFreq>>> alignedEntryStream = parallel
                ? align.entrySet().parallelStream()
                : (/*
        ...
        alignedEntryStream.forEach(e -> {
            ...
            if (algo instanceof Algorithm)
                deduped = ((Algorithm) algo).apply(e.getValue(), (DataStructure) data, currTracker, umiLength, k,
                        percentage);
            else
         ...
         * 
         * SamReader reader2 =
         * SamReaderFactory.makeDefault().validationStringency(ValidationStringency.
         * SILENT)
         * .open(in);
         * 
         ...
         * continue;
         * 
         * if (paired && ((removeUnpaired && !record.getReadPairedFlag()) // discard
         * unpaired
         * || (record.getReadPairedFlag() && record.getSecondOfPairFlag()) // ignore
         * reversed reads
         * || (record.getReadPairedFlag() && record.getMateUnmappedFlag()) // discard
         * unmapped reads
    ...
    public void deduplicateAndMergeTwoPass(File in, File out, Algo algo, Class<? extends Data> dataClass, Merge merge,
            int umiLengthParam, int k, float percentage, String umiSeparator, boolean paired, boolean removeUnpaired,
            boolean removeChimeric, boolean keepUnmapped, boolean trackClusters) {
        ...
        for (SAMRecord record : reader) {
            ...
            if (paired && ((removeUnpaired && !record.getReadPairedFlag()) // discard unpaired
                    ...
                    || (removeChimeric && record.getReadPairedFlag()
                            && !record.getReferenceName().equals(record.getMateReferenceName())))) { // discard chimeric
                                                                                                     // reads
                continue;
            }
            ...
            if (idx >= alignReads.latest) {
                ...
                if (algo instanceof Algorithm)
                    deduped = ((Algorithm) algo).apply(alignReads.umiRead, (DataStructure) data,
                            new ClusterTracker(trackClusters), umiLength, k, percentage);
                else
                    deduped = ((ParallelAlgorithm) algo).apply(alignReads.umiRead, (ParallelDataStructure) data,
                            new ClusterTracker(trackClusters), umiLength, k, percentage);
